...
# === Prompt 模板 ===
prompt_format = """### Instruction: You are a local expert from {region_name}. 
Is the following statement factually correct based on local knowledge? 
...
for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing"):
    ...
    orig_question = str(row["question"]).strip()
    for opt in ["A", "B", "C", "D"]:
        orig_option = str(row[f"option_{opt}"]).strip()
        # 直接拼接原始语言的 question + option
        statement = f"{orig_question} {orig_option}"
        pred = judge_statement(statement, lang_region)
        results.append({
            "id": q_id,
            "option_letter": opt,
            "prediction": pred
...
correct_df = result_df[result_df["prediction"] == 1]
df_final = (
    correct_df.groupby("id")["option_letter"]
    .apply(lambda x: " ".join(sorted(x)))
    .reset_index()
    .rename(columns={"option_letter": "prediction"})
)

all_ids = df["id"].unique()
df_final = df_final.set_index("id").reindex(all_ids).reset_index()

df_final["prediction"] = df_final["prediction"].fillna(" ")
